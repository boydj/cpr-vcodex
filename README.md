...
| Current release (CPR-vCodex) build | [`1.5.0.6-cpr-vcodex`](https://github.com/franssjz/cpr-vcodex/releases/tag/1.5.0.6-cpr-vcodex) |
...
| Current release focus | Makes Wi-Fi connection failures dismiss cleanly instead of trapping automatic-connect flows in a retry loop. |
| Latest release notes | - Back returns directly from a connection failure to the cached network list.<br>- Forget is offered only when a saved credential failed, and cancelling it no longer triggers another scan.<br>- Automatic connection is attempted only once per Wi-Fi selection session, so an explicit rescan remains under user control. |
